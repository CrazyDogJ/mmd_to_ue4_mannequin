...
class CDTOOLS_UL_BonesRenameList(bpy.types.UIList):
...
class CDTOOLS_UL_BonesReparentList(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            row = layout.row(align=True)
            row.prop(item, "bone1", icon='BONE_DATA', text="")
...
class CDTOOLS_PT_BoneMappingPanel(bpy.types.Panel):
    bl_label = "Bones Rename or Reparent Mapping"
    bl_idname = "VIEW3D_PT_bones_mapping_panel"
    ...
    def draw(self, context):
        ...
        row.template_list("CDTOOLS_UL_BonesRenameList", "rename_mapping", scene, "cdtools_mapping_items", scene, "cdtools_mapping_index", rows=7)
        ...
        col.template_list("CDTOOLS_UL_BonesReparentList", "reparent_mapping", scene, "cdtools_reparent_mapping_items", scene, "cdtools_reparent_mapping_index")

        col = row_reparent.column(align=True)
        add_item_op = col.operator("cdtools_bones_pair.add_item", icon="ADD", text="")
        add_item_op.target_var = "cdtools_reparent_mapping_items"
        add_item_op.target_index = "cdtools_reparent_mapping_index"

        remove_item_op = col.operator("cdtools_bones_pair.remove_item", icon="REMOVE", text="")
        remove_item_op.target_var = "cdtools_reparent_mapping_items"
        remove_item_op.target_index = "cdtools_reparent_mapping_index"

        col.separator()

        default_op = col.operator("cdtools_bones_pair.load_from_default", icon="FILE_REFRESH", text="")
        default_op.target_var = "cdtools_reparent_mapping_items"
        default_op.target_index = "cdtools_reparent_mapping_index"
        default_op.target_default_list = "reparent_list"

        import_op = col.operator("cdtools_bones_pair.import_from_file", icon="IMPORT", text="")
        import_op.target_var = "cdtools_reparent_mapping_items"
        import_op.target_index = "cdtools_reparent_mapping_index"

        col.operator("cdtools_bones_pair.clear_mapping", icon="X", text="").target_var = "cdtools_reparent_mapping_items"

        col.separator()
        
        col.operator("cdtools_bones_pair.reparent", icon="EVENT_RIGHT_ARROW", text="")
...
class CDTOOLS_OT_BonesRename(bpy.types.Operator):
...
class CDTOOLS_OT_BonesReparent(bpy.types.Operator):
    bl_idname = "cdtools_bones_pair.reparent"
    ...
    def execute(self, context):
        ...
        if selected_obj.type == 'ARMATURE':
            ...
            bpy.ops.object.mode_set(mode='EDIT')
            armature = selected_armature.data
            for item in scene.cdtools_reparent_mapping_items:
                bind_bone_to_parent(armature_data=armature, child_bone_name=item.bone1, parent_bone_name=item.bone2)
        return{'FINISHED'}
...
classes = {
    ...
    CDTOOLS_UL_BonesRenameList,
    CDTOOLS_UL_BonesReparentList,

    CDTOOLS_PT_BoneMappingPanel,
    ...
    CDTOOLS_OT_BonesRename,
    CDTOOLS_OT_BonesReparent,
}
...
def register():
    ...
    bpy.types.Scene.cdtools_reparent_mapping_items = bpy.props.CollectionProperty(type=CDTOOLS_PairBonesGroupProperty)
    bpy.types.Scene.cdtools_reparent_mapping_index = bpy.props.IntProperty(default=0)
...
def unregister():
    ...
    del bpy.types.Scene.cdtools_reparent_mapping_items
    del bpy.types.Scene.cdtools_reparent_mapping_index